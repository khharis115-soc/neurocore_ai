...
if "authenticated" not in st.session_state:
    ...
    with t2:
        ...
        if st.button("Register"):
            try: manage_db("INSERT INTO users VALUES (?, ?)", (re, hashlib.sha256(rp.encode()).hexdigest())); st.success("Done!")
...
# Display Chat
chat_history = manage_db("SELECT role, content FROM messages WHERE email=? AND session_id=? ORDER BY timestamp", (st.session_state.user_email, st.session_state.current_session), True)
for r, c in chat_history:
    with st.chat_message(r): st.markdown(c)

# Inputs
with st.container():
    up_file = st.file_uploader("Attach file", type=['png','jpg','pdf','docx','txt'], key=f"file_{st.session_state.reset_key}")
    c1, c2 = st.columns([1, 6])
    with c1:
        v_text = speech_to_text(language='en', start_prompt="🎤", stop_prompt="🛑", key=f"mic_{st.session_state.reset_key}")
    with c2:
        u_msg = st.chat_input("Ask anything...")
        ...
        with st.spinner("Thinking..."):
            if up_file:
                if up_file.type.startswith("image/"):
                    ...
                    res = st.session_state.neuro_engine.process_query(final_prompt, file_context=txt, history_context=history_str)
            else:
